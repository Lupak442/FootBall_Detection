...
def main():
    ...
    demo_selected = st.sidebar.radio(
        label="Select Demo Video", options=["Demo 1", "Demo 2"], horizontal=True
    )
    ...
    demo_vid_paths = {
        "Demo 1": r"C:\Users\user\Desktop\real_final_foot\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Streamlit web app\demo_vid_1.mp4",
        "Demo 2": r"C:\Users\user\Desktop\real_final_foot\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Streamlit web app\demo_vid_2.mp4",
    }
    ...
    model_players = YOLO(
        r"C:\Users\user\OneDrive - 호서대학교\딥러닝 project\football_Analytics\Football-Analytics-with-Deep-Learning-and-Computer-Vision\models\Yolo8L Players\weights\best.pt"
    )
    # Load the YOLOv8 field keypoints detection model
    model_keypoints = YOLO(
        r"C:\Users\user\OneDrive - 호서대학교\딥러닝 project\football_Analytics\Football-Analytics-with-Deep-Learning-and-Computer-Vision\models\Yolo8M Field Keypoints\weights\best copy.pt"
    )

    st.sidebar.markdown("---")
    st.sidebar.subheader("Team Names")
    team1_name = st.sidebar.text_input(
        label="First Team Name", value=selected_team_info["team1_name"]
    )
    team2_name = st.sidebar.text_input(
        label="Second Team Name", value=selected_team_info["team2_name"]
    )
    st.sidebar.markdown("---")

    ## Page Setup
    tab1, tab2, tab3 = st.tabs(
        ["How to use?", "Team Colors", "Model Hyperparameters & Detection"]
    )
    with tab1:
    ...
    with tab2:
        ...
        with t1col1:
            ...
            st.markdown("---")
            radio_options = [
                f"{team1_name} P color",
                f"{team1_name} GK color",
                f"{team2_name} P color",
                f"{team2_name} GK color",
            ]
            active_color = st.radio(
                label="Select which team color to pick from the image above",
            ...
            with cp1:
                hex_color_1 = (
                    st.session_state[f"{team1_name} P color"]
                    if f"{team1_name} P color" in st.session_state
                    else selected_team_info["team1_p_color"]
                )
            ...
            with cp2:
                hex_color_2 = (
                    ...
                    if f"{team1_name} GK color" in st.session_state
                    else selected_team_info["team1_gk_color"]
                )
                team1_gk_color = st.color_picker(
                    label=" ", value=hex_color_2, key="t1gk"
                )
                st.session_state[f"{team1_name} GK color"] = team1_gk_color
            with cp3:
                hex_color_3 = (
                    st.session_state[f"{team2_name} P color"]
                    if f"{team2_name} P color" in st.session_state
                    else selected_team_info["team2_p_color"]
                )
            ...
            with cp4:
                hex_color_4 = (
                    ...
                    if f"{team2_name} GK color" in st.session_state
                    else selected_team_info["team2_gk_color"]
                )
                team2_gk_color = st.color_picker(
                    label=" ", value=hex_color_4, key="t2gk"
                )
    ...
    with tab3:
        ...
        with t2col1:
            player_model_conf_thresh = st.slider(
                "PLayers Detection Confidence Threshold",
                min_value=0.0,
                max_value=1.0,
                value=0.6,
            )
            keypoints_model_conf_thresh = st.slider(
                "Field Keypoints PLayers Detection Confidence Threshold",
                min_value=0.0,
                max_value=1.0,
                value=0.7,
            )
            keypoints_displacement_mean_tol = st.slider(
                "Keypoints Displacement RMSE Tolerance (pixels)",
                min_value=-1,
                max_value=100,
                value=7,
        ...
        with t2col2:
            num_pal_colors = st.slider(
                label="Number of palette colors",
                min_value=1,
                max_value=5,
                step=1,
                value=3,
        ...
        with bcol1:
            nbr_frames_no_ball_thresh = st.number_input(
            ...
            ball_track_dist_thresh = st.number_input(
            ...
            max_track_length = st.number_input(
        ...
        with bcol2:
            ...
            plot_hyperparams = {0: show_k, 1: show_pal, 2: show_b, 3: show_p}
            st.markdown("---")
