...
def get_labels_dics():
    ...
    json_path = r"C:\Users\user\Desktop\real_final_foot\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Football-Analytics-with-Deep-Learning-and-Computer-Vision\pitch map labels position.json"
    with open(json_path, "r", encoding="utf-8") as f:
        keypoints_map_pos = json.load(f)

    # Get football field keypoints numerical to alphabetical mapping
    yaml_path = r"C:\Users\user\Desktop\real_final_foot\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Football-Analytics-with-Deep-Learning-and-Computer-Vision\config players dataset.yaml"
...
def create_colors_info(
    team1_name, team1_p_color, team1_gk_color, team2_name, team2_p_color, team2_gk_color
):
    ...
    colors_list = (
        colors_dic[team1_name] + colors_dic[team2_name]
...
def detect(
    ...
    tac_map = cv2.imread(
        r"C:\Users\user\Desktop\real_final_foot\Football-Analytics-with-Deep-Learning-and-Computer-Vision\Football-Analytics-with-Deep-Learning-and-Computer-Vision\tactical map.jpg"
    )
    ...
    if save_output:
        ...
        output = cv2.VideoWriter(
            f"./outputs/{output_file_name}.mp4",
        ...
        if success:
            ...
            if "homog" in locals():
                ...
                if detected_ball_src_pos is not None:
                    pt = np.append(
                        np.array(detected_ball_src_pos), np.array([1]), axis=0
                    )
            ...
            for i, j in enumerate(list(results_players[0].boxes.cls.cpu().numpy())):
                if int(j) == 0:
                    ...
                    center_filter = obj_img[
                        center_filter_y1:center_filter_y2,
            ...
            ball_color_bgr = (
                ...
                0,
                255,
            )  # Color (GBR) for ball annotation on tactical map
            ...
            for i in range(bboxes_p.shape[0]):
                ...
                if (
                    labels_p[i] == 0
                ):  # Display annotation for detected players (label 0)
                    ...
                    color_rgb = colors_dic[team_name][
                        0
                    ]  # Get detected player team color
            ...
            prev_frame_time = (
                new_frame_time  # Save current time to be used in next frame
            )
            cv2.putText(
                final_img,
                "FPS: " + str(int(fps)),
